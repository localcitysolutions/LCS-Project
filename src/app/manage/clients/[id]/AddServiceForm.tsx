...
export default function AddServiceForm({
  ...
  const labelClass = "block text-ink/40 text-xs mb-1";
  ...
  return (
    <form action={formAction} className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
      ...
      {state.error && <p className="text-red-600 text-xs sm:col-span-2">{state.error}</p>}
      <div className="sm:col-span-2">
        <button
          type="submit"
          disabled={pending}
          className="px-4 py-2 rounded-full bg-[#F5C518] text-ink font-bold text-xs hover:bg-[#F5C518]/90 transition-all disabled:opacity-50"
        >
